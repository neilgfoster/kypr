#!/usr/bin/env python3
"""contribute.py — path scope check and semver classifier for Hedl self-improvement PRs.

Used by the /contribute flow to validate that a proposed change:
  1. Only touches files under skill/hedl/ (path-prefix scope check — confirms no
     consumer source or non-framework files are included; does not read file contents).
...
CLI_SPEC: dict[str, Any] = {
    ...
    "description": "Path scope check and semver classifier for Hedl self-improvement PRs.",
...
def main() -> int:
    ...
    scrub_p = sub.add_parser("scrub", help="Path scope check for diff files (filename prefix only, no content inspection)")
