...
def _matches_any_glob(filepath: str, patterns: list[str], repo_root: str) -> bool:
    ...
    Uses path-string matching (fnmatch) so an exclude is deterministic
    regardless of which files exist on disk (WORK-0063).
    """
    try:
...
def validate_file(filepath: str, schema: dict[str, Any], repo_root: str) -> list[dict[str, Any]]:
    ...
    if fm_schema is not None:
        ...
        if frontmatter is None:
            ...
            for field_name in required_fields:
                ...
                if "pattern" in rules:
                    try:
                        matched = re.search(rules["pattern"], str(value))
                    except re.error as exc:
                        add(field_name, f"schema regex error in '{field_name}' pattern: {exc}")
                    else:
                        if not matched:
    ...
    if "title_pattern" in schema:
        ...
        if heading_line is None:
            ...
            try:
                matched = re.match(schema["title_pattern"], heading_line)
            except re.error as exc:
                add("title", f"schema regex error in title_pattern: {exc}")
            else:
                if not matched:
    ...
    if "required_patterns" in schema:
        for pat_def in schema["required_patterns"]:
            ...
            try:
                ...
                add("pattern", f"schema regex error in required_patterns '{desc}': {exc}")
            else:
                if not found:
