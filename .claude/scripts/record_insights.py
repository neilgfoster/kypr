...
def _reviewer_names(project_dir: Path) -> frozenset[str]:
...
def _tool_to_event(
    ...
    if tool_name in ("Agent",) and reviewers:
        # Agent invocations may be reviewer calls; surface as reviewer_fired
        # if description contains a known reviewer name.
        # Note: this branch only fires when the hook is wired to an Agent-matching
        # PostToolUse entry. The Edit|Write hook wiring does not cover Agent calls.
...
def main() -> int:
    ...
    event = _tool_to_event(payload, _reviewer_names(project_dir))
    if event is None:
        return 0
